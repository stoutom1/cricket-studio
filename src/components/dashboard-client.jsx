...
useEffect(() => {
  if (
    ...
    setPlayerForm((prev) => ({
      ...
      teamId: activeLeague.teams[0].id,
    }));
  }
...
const handleBulkAddPlayers = async (e) => {
  ...
  try {
    const players = playerForm.playersText
      .split("\n")
      .map((p) => p.trim())
      .filter(Boolean);
...
<div className="dashboard-tabs">
  ...
  {permissions?.canViewMatches && (
    ...
    >
      <span className="tab-icon">🎛️</span>
      <span className="tab-text">Match Day</span>
    </button>
  )}
...
{activeLeague && (
  <Card
    title={`🏏 Teams (${activeLeague.teams?.length || 0})`}
  >
...
MS Dhoni
KL Rahul`}
            value={playerForm.playersText}
            onChange={(e) =>
              setPlayerForm((prev) => ({
                ...prev,
                playersText: e.target.value,
              }))
            }
            required
          />
